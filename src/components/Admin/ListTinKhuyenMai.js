...
import ProductTableTinKhuyenMai from './ProductTableTinKhuyenMai';
...
const getAllTinKhuyenMai = (limit) =>
...
class ListTinKhuyenMai extends Component {
    constructor(props) {
        super(props);
        this.state = {
            inputValue: null,
        ...
        if (!token) {
            ...
            checkUser(token).then((result) => {
                if (result.status === true) {
                    ...
                    getAllTinKhuyenMai(100).then((result) => {
                        this.setState({
        ...
        return (
            <React.Fragment>
                <div className="wrapper st-body">
                    <HeaderBooking />
                    <h1>Tin tức</h1>
                    <ProductTableTinKhuyenMai getAgainData={this.getAgainData.bind(this)} inputValue={this.state.inputValue} onProductTableUpdate={this.handleProductTable.bind(this)} onRowDel={this.handleRowDel.bind(this)} products={this.state.products} filterText={this.state.filterText} />
                    <Footer />
                </div>
            </React.Fragment>
        );
...
export default ListTinKhuyenMai;